...
| `declarative-settings-api.md` | Building a settings tab with `getSettingDefinitions()` (Obsidian 1.13+) — covers when definitions are re-evaluated, `refreshDomState()` vs `update()`, cascading side effects without an `onChange`, row reuse that duplicates hand-mounted DOM, what breaks search indexing, and the `renderTab` name collision that blanks the settings pane. |
